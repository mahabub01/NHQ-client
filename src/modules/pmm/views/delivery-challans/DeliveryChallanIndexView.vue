  ...
  <div>
                ...
                <div class="col-md-7">
                  <div class="page-bootcamp-right">
                    <div>
                      ...
                      <input
                        type="text"
                        placeholder="Search By Delivery Challan Title"
                        style="margin-right: 7px"
                        v-model.lazy="search"
                        class="table-search"
                      />
    ...
    <div>
      <create-modal :modalsize="modalSize" v-if="createModalState">
        <template v-slot:header
          ><i class="fas fa-plus-square"></i> Create Delivery Challan
        </template>
    ...
    <EditModal :modalsize="modalSize" v-if="editModalState">
      <template v-slot:editheader
        ><i class="fas fa-plus-square"></i> Edit Delivery Challan
      </template>
...
import DeliveryChallanTable from "./DeliveryChallanTable.vue";
...
//filter by DeliveryChallan ID/ DeliveryChallan title
watch([search], async () => {
  // alert(search.value);
  filterData(
    "/projects/delivery-challans",
...
async function updateHandler() {
  ...
  if (!v$.value.$error) {
    ...
    savingSpinner.value = true;
    await Axios.put(
      "projects/delivery-challans/" + editable_id.value,
      formState
    )
      .then((response) => {
        if (response.data.code === 200) {
...
function remove(id: number) {
  swal({
    ...
    if (willDelete) {
      ...
      await Axios.delete("/projects/delivery-challans/" + id).then(
        (response) => {
...
function bulkDelete() {
  ...
  swal({
    ...
    if (willDelete) {
      deletingSpinner.value = true;
      await Axios.post("/projects/delivery-challans-multidelete", {
