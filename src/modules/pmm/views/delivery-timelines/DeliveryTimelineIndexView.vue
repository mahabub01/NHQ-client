  ...
  <div>
                ...
                <div class="col-md-7">
                  <div class="page-bootcamp-right">
                    <div>
                      ...
                      <input
                        type="text"
                        placeholder="Search By Delivery Timeline Title"
                        style="margin-right: 7px"
                        v-model.lazy="search"
                        class="table-search"
                      />
                      ...
                      <button
                        v-if="getPermission(`create_delivery_timeline`)"
                        type="button"
                        class="link_btn"
                        style="margin-right: 7px"
                        @click="
                      ...
                      <div class="btn-group">
                        <button
                          v-if="getPermission(`bulk_delete_delivery_timeline`)"
                          type="button"
    ...
    <div>
      <create-modal :modalsize="modalSize" v-if="createModalState">
        <template v-slot:header
          ><i class="fas fa-plus-square"></i> Create Delivery Timeline
        </template>
    ...
    <EditModal :modalsize="modalSize" v-if="editModalState">
      <template v-slot:editheader
        ><i class="fas fa-plus-square"></i> Edit Delivery Timeline
      </template>
...
import DeliveryTimelineTable from "./DeliveryTimelineTable.vue";
...
//filter by DeliveryTimeline ID/ DeliveryTimeline title
watch([search], async () => {
  filterData(
    "/projects/delivery-timelines",
...
async function submitHandler() {
  ...
  if (!v$.value.$error) {
    ...
    await Axios.post("projects/delivery-timelines", formState)
      .then((response) => {
...
async function editModal(id: number) {
  ...
  await Axios.get("/projects/delivery-timelines/" + id).then((response) => {
...
async function updateHandler() {
  ...
  if (!v$.value.$error) {
    ...
    savingSpinner.value = true;
    await Axios.put(
      "projects/delivery-timelines/" + editable_id.value,
      formState
    )
      .then((response) => {
        if (response.data.code === 200) {
...
function remove(id: number) {
  swal({
    ...
    if (willDelete) {
      ...
      await Axios.delete("/projects/delivery-timelines/" + id).then(
        (response) => {
...
function bulkDelete() {
  ...
  swal({
    ...
    if (willDelete) {
      deletingSpinner.value = true;
      await Axios.post("/projects/delivery-timelines-multidelete", {
