            ...
            <div class="row">
              <div class="col-md-4 mb_30">
                ...
                <input
                  type="text"
                  class="form-input"
                  v-model="filterState.project_name_id"
                />
              </div>
...
onMounted(async () => {
  filterData(
    "/employee-project",
    "&user_id=" + route.params.user_id + "&flag=" + userInfo.value.flag
  );
});
...
async function filterSubmit() {
  ...
  await Axios.post("employee-project-filter", filterState).then((response) => {
