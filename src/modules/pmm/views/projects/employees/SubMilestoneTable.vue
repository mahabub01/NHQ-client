    ...
    <table class="table" id="selectable-table">
      <thead>
        <tr>
          ...
          <th style="width: 200px">Sub Milestone Name</th>
          <th style="width: 80px">Sub Milestone ID</th>
          <th>Expected</th>
          <th>Actual</th>
          <th>Start Date</th>
          <th>End Date</th>
      ...
      <tbody :class="{ tableLoader: $attrs.loadingState }">
        <tr v-for="(item, index) in $attrs.entries" :key="index">
          ...
          <td style="width: 200px !important">
            <router-link
              :to="`/pmm/milestones/${route.params.user_id}/${item.id}`"
              >{{ item.submilestone_name }}</router-link
            >
          </td>
          <td style="width: 80px">{{ item.submilestone_unique_id }}</td>
          ...
          <td>
            <div class="progress" style="height: 14px; position: relative">
              <div
                ...
                role="progressbar"
                :style="`width: ${item.progress}%; background-color: #017e84 !important`"
