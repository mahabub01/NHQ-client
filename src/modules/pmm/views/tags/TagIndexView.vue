    ...
    <div>
      <create-modal>
        <template v-slot:header
          ><i class="fas fa-plus-square"></i> Create Tag
        </template>
        <template v-slot:body>
          <form @submit.prevent="tagSubmit" class="form-page">
            <div class="row">
              <div class="col-md-12">
...
function remove(id: number) {
  swal({
    ...
    if (willDelete) {
      ...
      await Axios.delete("/projects/tags/" + id).then((response) => {
...
function bulkDelete() {
  ...
  swal({
    ...
    if (willDelete) {
      deletingSpinner.value = true;
      await Axios.post("/projects/tags-multidelete", {
...
async function changeStatus(status: { id: number; status: number }) {
  await Axios.post("/projects/tags-status", status).then((response) => {
